#!/usr/bin/env python3
"""
Modify Images_workflow.json with image prompts, video prompts, AND name
"""
...
workflow['443']['inputs']['value'] = image_prompts  # Image Prompts
workflow['500']['inputs']['value'] = video_prompts  # Video Prompts (titled "Name" in UI)

# Find and update the name field - it's likely a simple text input
# Looking for node with "robe" value and changing it to script name
for node_id, node in workflow.items():
    if 'inputs' in node:
        for key, val in node['inputs'].items():
            if isinstance(val, str) and val == "robe":
                workflow[node_id]['inputs'][key] = script_name
                print(f"[OK] Updated Node {node_id} ({node.get('_meta', {}).get('title', '?')}): name = {script_name}")
...
print(f"[OK] Name field: {script_name}")
