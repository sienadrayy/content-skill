#!/usr/bin/env python3
"""
ComfyUI Workflow Converter - Corrected version with proper link remapping.
"""
...
class ComfyUIConverter:
    ...
    def convert(self, prompt_text=None) -> Dict:
        ...
        api = {}
        sg_info = {}  # Track subgraph nodes and their outputs
        ...
        for node in self.wf['nodes']:
            if node['type'] not in self.subgraphs:
                self._add_regular_node(api, node, links)
    ...
    def _add_regular_node(self, api: Dict, node: Dict, links: Dict):
        """Add a regular (non-subgraph) node to API."""
        nid = str(node['id'])
        api_node = {
            'inputs': {},
        ...
        remaining = node.get('widgets_values', [])[widget_idx:]
    ...
    def _flatten_sg(self, api: Dict, sg_def: Dict, ext_inputs: Dict) -> Dict:
        """
        Flatten a subgraph.
        Returns: mapping of subgraph output index -> (new_node_id, slot)
        """
        
        # Build internal link map
        sg_links = {link['id']: link for link in sg_def.get('links', [])}
        
        # Step 1: Copy internal nodes and build ID map
        internal_to_new = {}
        ...
        # Step 2: Create API nodes with remapped links
        for orig_node in sg_def['nodes']:
            ...
            for inp in orig_node.get('inputs', []):
                if 'widget' in inp and inp['widget']:
                    inp_name = inp.get('name')
                    if widget_idx < len(orig_node.get('widgets_values', [])):
                        api_node['inputs'][inp_name] = orig_node['widgets_values'][widget_idx]
                        widget_idx += 1
            
            # Add remaining widgets
            remaining = orig_node.get('widgets_values', [])[widget_idx:]
            for i, val in enumerate(remaining):
                api_node['inputs'][f'widget_{i}'] = val
    ...
    def submit(self, api_wf: Dict, server="http://192.168.29.60:8188"):
        ...
        # Remove MarkdownNote
        if '35' in api_wf and api_wf['35'].get('class_type') == 'MarkdownNote':
            del api_wf['35']
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                try:
                    error = resp.json()
                    if 'node_errors' in error and error['node_errors']:
                        print(f"  Node errors:")
                        for nid, err in error['node_errors'].items():
...
def main():
    print("=" * 70)
    print("ComfyUI Workflow Converter - Corrected Version")
    print("=" * 70)
