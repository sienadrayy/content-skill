#!/usr/bin/env python3
"""
Submit dual workflows (Image + Video) to ComfyUI with parameters
"""
...
WORKFLOW_IMAGE_PATH = r"C:\Users\mohit\.openclaw\workspace\comfy-wf\openclaw\Images_workflow.json"
...
def submit_image_workflow(image_prompts: str, output_name: str) -> str:
    """Submit Image workflow"""
    print(f"📤 Loading Image workflow...")
    workflow = load_workflow(WORKFLOW_IMAGE_PATH)
    
    # Set inputs
    workflow["443"]["inputs"]["value"] = image_prompts
    workflow["500"]["inputs"]["value"] = output_name
    
    print(f"🖼️  Setting image prompts and output name: {output_name}")
    ...
    # Submit
    print(f"📤 Submitting Image workflow to ComfyUI...")
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            print(f"✅ Image workflow submitted!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Submit dual workflows (Image + Video) to ComfyUI")
    ...
    try:
        print(f"\n{'='*60}")
        print(f"🎬 DUAL WORKFLOW SUBMISSION")
        print(f"{'='*60}")
        ...
        # Submit Image
        print(f"\n{'='*60}")
        print(f"STEP 1: Submit Image Workflow")
        print(f"{'='*60}")
        image_prompt_id = submit_image_workflow(args.image_prompts, args.name)
        
        # Wait 5 seconds
        print(f"\n⏳ Waiting 5 seconds before submitting Video workflow...")
        time.sleep(5)
        
        # Submit Video
        print(f"\n{'='*60}")
        print(f"STEP 2: Submit Video Workflow")
        print(f"{'='*60}")
        video_prompt_id = submit_video_workflow(args.video_prompts, args.name, args.seconds)
        
        # Summary
        print(f"\n{'='*60}")
        print(f"🎉 BOTH WORKFLOWS SUBMITTED!")
        print(f"{'='*60}")
        print(f"Output Name: {args.name}")
        ...
        print(f"💡 Both workflows running in parallel on ComfyUI")
