#!/usr/bin/env python3
"""Submit workflow with specific image path"""
...
image_path = r"D:\ComfyUI_windows_portable\ComfyUI\output\ComfyUI_00001_.png"

print("=" * 70)
print("ComfyUI Export API - With Image Path")
print("=" * 70)
...
# Update image path if any LoadImage nodes exist
print("\n5. Setting image path...")
for node_id, node_data in api_prompt.items():
    class_type = node_data.get('class_type', '')
    
    # Update LoadImageFromPath nodes
    if class_type == 'LoadImageFromPath':
        if 'image' in node_data.get('inputs', {}):
            node_data['inputs']['image'] = image_path
            print(f"   Set image path in node {node_id}")
...
# Save
print("\n7. Saving converted workflow...")
with open('Images_workflow_with_image_path.json', 'w') as f:
    json.dump(api_prompt, f, indent=2)
print(f"   Saved to: Images_workflow_with_image_path.json")
